...
const useChecked = <T extends Record<string, any>>(dataSource: T[], { key = 'id' }: Option = {}) => {
	...
	const checkedAll = useMemo(() => dataSource.length !== 0 && filterChecked().length === dataSource.length, [ dataSource, filterChecked ])

	return { checkedMap, checkedAll, onCheckAllChange, onCheckedChange, filterChecked }
}
